//SPDX-License-Identifier: UNLICENSED
 ...
 contract FlashBotsUniswapQuery {
     function getReservesByPairs(IUniswapV2Pair[] calldata _pairs) external view returns (uint256[5][] memory) {
     ...
     for (uint i = 0; i < _pairs.length; i++) {
         ...
         address token1 = _pairs[i].token1();

         result[i][3] = IERC20(token0).decimals();
         result[i][4] = IERC20(token1).decimals();
     }
